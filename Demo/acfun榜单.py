...
mian_url = 'https://www.acfun.cn/rest/pc-direct/rank/channel?channelId=&subChannelId=&rankLimit=30&rankPeriod=DAY'
mian_res = requests.get(url=mian_url, headers=headers).json()
rankList = mian_res['rankList']
for lst in rankList:
    dougaId = lst['dougaId']
    main_title = lst['title']
    main_title = re.sub(r'[/\:*?"<>|\n]', '', main_title)
    # print(dougaId, main_title)
    url = f'https://www.acfun.cn/v/ac{dougaId}'
    ...
    for ts in res_info:
        ...
        title = ts.split('.')[1]
        with open(f'video/{main_title}.mp4', 'ab') as f:
            f.write(content)
        print(title_info, name, title, ts_url)
    print(f'{main_title}---下载完成'.center(50, '-'))
